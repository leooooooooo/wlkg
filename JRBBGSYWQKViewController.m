...
@interface JRBBGSYWQKViewController ()
@property (nonatomic,retain) NSArray *ChartDate;
@end

@implementation JRBBGSYWQKViewController
...
-(void)loadChartAndTabel
{
    ...
    NSString *bodyStr = [NSString stringWithFormat:@"ServiceNum=%@",@"12"];
    //将nstring转换成nsdata
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *name = [[NSArray alloc]initWithObjects:@"入驻企业",@"进库量",@"出库量",@"港存量", nil];
    //Date
    ((UILabel *)[cell.contentView viewWithTag:1]).text = [name objectAtIndex:indexPath.row];
    //Fund
    NSMutableAttributedString *str =[[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@ %@",[self.ChartDate objectAtIndex:indexPath.row],indexPath.row==0?@"家":@"吨"]];
