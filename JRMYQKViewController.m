...
@interface JRMYQKViewController ()
@property (nonatomic,retain) NSArray *ChartDate;
@end

@implementation JRMYQKViewController
...
-(void)loadChartAndTabel
{
    ...
    NSString *bodyStr = [NSString stringWithFormat:@"ServiceNum=%@",@"9"];
    //将nstring转换成nsdata
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError * error) {
        ...
        if (data != nil) {  //接受到数据，表示工作正常
            ...
            NSLog(@"%@", Info);
            self.ChartDate = [Info objectForKey:@"TodayTrade"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *name = [[NSArray alloc]initWithObjects:@"购进量",@"销售量",@"杂散货港存量",@"木材港存量", nil];
    //Date
    ((UILabel *)[cell.contentView viewWithTag:1]).text = [name objectAtIndex:indexPath.row];
    //Fund
    NSMutableAttributedString *str =[[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@ %@",[self.ChartDate objectAtIndex:indexPath.row],indexPath.row==3?@"立方米":@"吨"]];
    [str addAttribute:NSForegroundColorAttributeName value:[UIColor redColor] range:NSMakeRange(0,str.length-(indexPath.row==3?3:1))];
