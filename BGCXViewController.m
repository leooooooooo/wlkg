...
@interface BGCXViewController ()
@property(retain,nonatomic)NSArray *array;
@end

@implementation BGCXViewController
...
{
    //1确定地址NSURL
    NSString *urlString = [NSString stringWithFormat:@"http://218.92.115.55/wlkg/Service/Declare.aspx"];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError * error) {
        ...
        if (data != nil) {  //接受到数据，表示工作正常
            ...
            NSLog(@"%@", Info);
            self.array = [Info objectForKey:@"Declare"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DateLabel.text = [NSString stringWithFormat:@"批次：%@\n报关类别：%@\n货主名称：%@\n船名：%@",[[self.array objectAtIndex:indexPath.row]objectAtIndex:0],[[self.array objectAtIndex:indexPath.row]objectAtIndex:1],[[self.array objectAtIndex:indexPath.row]objectAtIndex:2],[[self.array objectAtIndex:indexPath.row]objectAtIndex:3]];
    ...
    DateLabel1.text = [NSString stringWithFormat:@"船次：%@\n提单号：%@\n提单数：%@\n货物名称：%@",[[self.array objectAtIndex:indexPath.row]objectAtIndex:4],[[self.array objectAtIndex:indexPath.row]objectAtIndex:5],[[self.array objectAtIndex:indexPath.row]objectAtIndex:6],[[self.array objectAtIndex:indexPath.row]objectAtIndex:7]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSURL *url =[NSURL URLWithString:[NSString stringWithFormat:@"http://218.92.115.55/wlkg/Page/DeclareReported.html?info=%@+%@+%@",[(AppDelegate *)[[UIApplication sharedApplication]delegate]Code_User],[[self.array objectAtIndex:indexPath.row]objectAtIndex:8],[[self.array objectAtIndex:indexPath.row]objectAtIndex:5]]];
