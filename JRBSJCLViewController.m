...
@interface JRBSJCLViewController ()
@property (nonatomic,retain) NSArray *ChartDate;
@end

@implementation JRBSJCLViewController
...
-(void)loadChartAndTabel
{
    ...
    NSString *bodyStr = [NSString stringWithFormat:@"ServiceNum=%@",@"8"];
    //将nstring转换成nsdata
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError * error) {
        ...
        if (data != nil) {  //接受到数据，表示工作正常
            ...
            NSLog(@"%@", Info);
            self.ChartDate = [Info objectForKey:@"TodayBondInAndOut"];
            
            
            //Tabel
            UITableView *tableview = [[UITableView alloc]initWithFrame:CGRectMake(0, 60,WIDTH, HEIGHT-60) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *name = [[NSArray alloc]initWithObjects:@"进",@"年累计进",@"出",@"年累计出", nil];
    //Date
    ((UILabel *)[cell.contentView viewWithTag:1]).text = [name objectAtIndex:indexPath.row];
    //Fund
    
    NSMutableAttributedString *str =[[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@ 吨",[self.ChartDate objectAtIndex:indexPath.section*4+indexPath.row]]];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    Date.text=[name objectAtIndex:section];
